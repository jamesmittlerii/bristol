...
#ifdef BRISTOL_LV2
int
bristolLv2SysexParam(audioMain *audiomain, int sid, int op, int ctrl,
	int value)
{
	bristolMidiMsg msg;

	bzero(&msg, sizeof(msg));
	msg.command = MIDI_SYSEX;
	msg.params.bristol.channel = sid;
	msg.params.bristol.operator = op;
	msg.params.bristol.controller = ctrl;
	msg.params.bristol.valueLSB = value & 0x7f;
	msg.params.bristol.valueMSB = (value >> 7) & 0x7f;
	msg.params.bristol.SysID = (audiomain->SysID >> 24) & 0xff;
	msg.params.bristol.L = (audiomain->SysID >> 16) & 0xff;
	msg.params.bristol.a = (audiomain->SysID >> 8) & 0xff;
	msg.params.bristol.b = audiomain->SysID & 0xff;
	msg.params.bristol.msgType = MSG_TYPE_PARAM;

	return(midiSystem(audiomain, &msg));
}
#endif /* BRISTOL_LV2 */
